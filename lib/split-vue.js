...
function getTemplatePosition(editor) {
  ...
  if (templateTagPosition.length) {
    ...
    return [[templateTagPosition[0].start.row + 1, templateTagPosition[0].start.column], [templateTagPosition[templateTagPosition.length - 1].end.row, templateTagPosition[templateTagPosition.length - 1].end.column]]
  }
  return null
}
...
function getScriptPosition(editor) {
  ...
  if (scriptTagPosition.length) {
    // add and subtract 1 to allow tags to show
    return [[scriptTagPosition[0].start.row + 1, scriptTagPosition[0].start.column], [scriptTagPosition[1].end.row, scriptTagPosition[1].end.column]]
  }
  return null
}
